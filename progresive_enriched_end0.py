...
def find_progressive_enriched_path(graph, start, end, available_nodes, 
    ...
    """
    [REFACTOR]
    Fungsi ini sekarang "Budget-Aware".
    - Menerima status anggaran (current_time, current_cost)
    - Menerima batasan anggaran (TIME_BUDGET, MONEY_BUDGET)
    - Melakukan Pengecekan Kelayakan (Feasibility Check) sebelum memilih simpul
    - Mengembalikan path, total waktu segmen, dan total biaya segmen
    """
    path, current = [start], start
    remaining_nodes = set(available_nodes)
    alpha, beta = 0.7, 0.3 # Bobot heuristik (bisa Anda sesuaikan)

    # [REFACTOR] Inisialisasi pelacak biaya untuk SEGMEN ini
    segment_time = 0.0
    segment_cost = 0.0
    ...
    while current != end:
        ...
        for competitor in competitors:
            ...
            # 3. [REFACTOR] Ambil data biaya & waktu dari simpul kompetitor
            #    Gunakan .get(key, 0) agar aman jika 'end' node tidak punya atribut
            service_time = graph.nodes[competitor].get('waktu_layanan', 0)
            price = graph.nodes[competitor].get('biaya', 0)

            # 4. [REFACTOR] === PEMERIKSAAN KELAYAKAN (CONSTRAINT CHECK) ===
            #    Apakah kita punya cukup waktu & uang untuk pergi ke 'competitor' INI
            #    DAN MASIH BISA lanjut ke 'end' (tujuan segmen)?
            
            # Total waktu = Waktu saat ini + Waktu tempuh ke C + Waktu layanan di C
            time_if_chosen = current_time_spent + segment_time + dist_curr_comp + service_time
            # Total biaya = Biaya saat ini + Biaya di C
            cost_if_chosen = current_cost_spent + segment_cost + price
            
            # Waktu total jika kita pilih C dan *langsung* lanjut ke 'end'
            ...
            if time_to_reach_segment_end > TIME_BUDGET or cost_if_chosen > MONEY_BUDGET:
                # Pilihan ini tidak layak (melebihi anggaran), abaikan
                continue 

            # 5. Jika layak, hitung skor heuristiknya (logika lama Anda)
            score = (alpha * dist_curr_comp) + (beta * dist_comp_end)
            if score < best_score:
                best_score, best_next_stop = score, competitor

        if best_next_stop is None:
            # Tidak ada kompetitor yang layak, kita harus langsung ke 'end'
            best_next_stop = end
            
            # Cek kelayakan terakhir untuk langsung ke 'end'
            travel_time_to_end = dijkstra(graph, current, end)
            if current_time_spent + segment_time + travel_time_to_end > TIME_BUDGET:
                 print(f"      -> PERINGATAN: Anggaran waktu habis. Tidak bisa mencapai '{end}'.")
                 return path, segment_time, segment_cost # Kembalikan path seadanya
        ...
        # [REFACTOR] Akumulasi total waktu segmen
        segment_time += travel_time_to_stop
        
        current = best_next_stop
        
        if current != end:
            # [REFACTOR] Tambahkan waktu layanan & biaya HANYA jika itu BUKAN 'end'
        ...
        if current == end:
            break # Kita sudah sampai di tujuan segmen

    # [REFACTOR] Kembalikan total waktu dan biaya yang dihabiskan di segmen ini
